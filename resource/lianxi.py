...
class ttt(QWidget):
    ...
    def readfile(self):
        ...
        if os.path.exists(self.savefilename):
            ...
            with open(self.savefilename, 'w', encoding='gbk') as f:
                self.all = {'like': [], 'allmusic': [], 'playlist': {}, 'musiclist': [], 'playingmusic': '',
                            'playmode': '随机播放', 'playingtime': 0, 'playhistory': {}, 'musicsec': 0, 'volume': 1.0}
    ...
    def mlmenu(self, pos):
        ...
        if self.ml.currentRow() >= 0:
            ...
            pls, pls34, pls35, pls36 = [], [menu34.addAction('like')], [menu35.addAction('like')], [
                menu36.addAction('like')]
            ...
            if action == item1:  # 播放
                ...
                self.operatorlist(action, self.all['musiclist'], pls, pls34, pls35, pls36, item4, item41, item42,
                                  item43, item5, item6)
            elif self.box1.isCheckable():
                self.operatorlist(action, self.all['allmusic'], pls, pls34, pls35, pls36, item4, item41, item42, item43,
                                  item5, item6)
            elif self.mlabel.text() in self.all['playlist'].keys():
                self.operatorlist(action, self.all['playlist'][self.mlabel.text()], pls, pls34, pls35, pls36, item4,
                                  item41, item42, item43, item5, item6)
            elif self.mlabel.text() == 'like':
                self.operatorlist(action, self.all['like'], pls, pls34, pls35, pls36, item4, item41, item42, item43,
                                  item5, item6)
    ...
    def operatorlist(self, action, list, pls, pls34, pls35, pls36, item4, item41, item42, item43, item5, item6):
        ...
        if action in pls:  # 添加到指定歌单
            ...
            for x in list:
                ...
                elif tx.artist == ti.artist and pls34.index(action) != 0 and x not in self.all['playlist'][
                    self.playlist[pls34.index(action) - 1]]:
            ...
            for x in list:
                ...
                if tx.album == ti.album and pls35.index(action) == 0 and x not in self.all['like']:
                    self.all['like'].insert(0, x)
                elif tx.album == ti.album and pls34.index(action) != 0 and x not in self.all['playlist'][
                    self.playlist[pls35.index(action) - 1]]:
            ...
            for x in list:
                ...
                if par == parents and pls36.index(action) == 0 and x not in self.all['like']:
                    self.all['like'].insert(0, x)
                elif par == parents and pls34.index(action) != 0 and x not in self.all['playlist'][
                    self.playlist[pls36.index(action) - 1]]:
